...
# 1. Read fresh restored file
with codecs.open('src/components/accounts/v2/AccountDetailHeaderV2.tsx', 'r', 'utf-8-sig') as f:
    lines = f.readlines()

# 2. Extract Prefix segments to avoid duplicates but keep initial logic and components
prefix_1 = lines[:292]  # up to end of selectedCycleMetrics
prefix_2 = lines[338:507] # from after cycleMetricSnapshot to return

# 3. Component logic enrichment (harmonious version)
logic_block = '''
...
# 4. Redesigned return statement - HARMONIOUS MASTER VERSION
              ...
              <HeaderSection label="Credit Health" borderColor="border-indigo-100" className="flex-[5] min-w-[400px] bg-indigo-50/10 cursor-help flex flex-col gap-0 py-2 border-dashed">
                ...
                <div className="flex items-center justify-between h-12 border-t border-slate-100/40 mt-1.5 pt-1.5 px-2">
                  ...
                  <div className="flex items-center gap-4">
                    <span className="text-[14px] font-black text-slate-700 tabular-nums bg-slate-50 px-3 py-0.5 rounded-lg border border-slate-100 shadow-sm">{formatMoneyVND(Math.ceil(summary?.yearExpensesTotal || 0))} / {formatMoneyVND(account.credit_limit || 0)}</span>
                  </div>
                </div>
                ...
                <div className="p-6 space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-indigo-50/50 rounded-2xl p-4 border border-indigo-100/50 group hover:bg-indigo-100/50 transition-colors">
                      ...
                      <div className="text-xl font-black text-indigo-900 tabular-nums">{formatMoneyVND(summary?.yearExpensesTotal || 0)}</div>
                    </div>
                  ...
                  {!!account.annual_fee_waiver_target && (
                    <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 space-y-4 shadow-inner">
                       <div className="flex justify-between items-center"><div className="flex flex-col gap-0.5"><span className="text-[11px] font-black text-slate-800 uppercase tracking-tight">Waiver Achievement</span><span className="text-[10px] font-medium text-slate-500 italic">Target: {formatMoneyVND(account.annual_fee_waiver_target)}</span></div><div className="text-lg font-black text-indigo-600">{(Math.min(100, ((summary?.yearExpensesTotal || 0) / account.annual_fee_waiver_target) * 100)).toFixed(0)}%</div></div>
                       <div className="h-2 w-full bg-slate-200 rounded-full overflow-hidden shadow-inner"><div className="h-full bg-gradient-to-r from-indigo-600 to-sky-400 shadow-[0_0_10px_rgba(79,70,229,0.3)] transition-all duration-1000" style={{ width: `${Math.min(100, ((summary?.yearExpensesTotal || 0) / account.annual_fee_waiver_target) * 100)}%` }} /></div>
...
with codecs.open('src/components/accounts/v2/AccountDetailHeaderV2.tsx', 'w', 'utf-8') as f:
    f.writelines(prefix_1)
    f.writelines(prefix_2)
    f.write(logic_block)
    f.write(redesigned_return)

print('Success! Harmonious Master build written.')
