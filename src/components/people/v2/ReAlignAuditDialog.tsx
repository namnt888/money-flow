...
interface ReAlignAuditDialogProps {
    open: boolean
...
export function ReAlignAuditDialog({
    open,
    onOpenChange,
    person,
    initialYear = '2026',
    availableYears = []
}: ReAlignAuditDialogProps) {
                    ...
                    <div className="flex items-center justify-between gap-4">
                        <div className="space-y-1">
                            <DialogTitle className="flex items-center gap-2 text-xl font-bold">
                                ...
                                Re-Align Audit: {person.name}
                            </DialogTitle>
                            <DialogDescription className="text-sm text-slate-500 font-medium">
                                Cross-check every debt record and repayment for reconciliation.
                            </DialogDescription>
                        </div>
                ...
                <div className="flex flex-col flex-1 overflow-hidden">
                    ...
                    <div className="flex-1 overflow-auto relative min-h-[400px]">
                        {isLoading ? (
                            <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-white/50 backdrop-blur-[1px] z-10 transition-all duration-300">
                                ...
                                <span className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">Calculating Ledger...</span>
                            </div>
                        ) : filteredTransactions.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-20 gap-4">
                                <AlertCircle className="h-10 w-10 text-slate-200" />
                                <span className="text-sm font-bold text-slate-400 italic">No transactions found for this period.</span>
                            </div>
                        ) : (
                                ...
                                <TableBody>
                                    {filteredTransactions.map((t, idx) => (
                                        ...
                                        >
                                                ...
                                                <div className="flex flex-col gap-0.5">
                                                    <div className="flex items-center gap-1.5">
                                                        ...
                                                        <span className="text-[11px] font-bold text-slate-700 truncate">
                                                            {t.note || (t.type === 'debt' ? 'Lend Money' : 'Repayment')}
                                                        </span>
                                                    </div>
                                                    {t.shop_name && (
                                            ...
                                            <TableCell className={cn(
                                                "text-right font-black text-[12px] tabular-nums border-r",
                                                t.type === 'debt' || t.type === 'expense' ? "text-rose-600" : "text-emerald-600"
                                            )}>
                                                {t.type === 'debt' || t.type === 'expense' ? '+' : '-'}{formatMoneyVND(t.finalPrice)}
                                            </TableCell>
                                            <TableCell className={cn(
                                                ...
                                                <div className="h-[1.5px] w-full bg-slate-100 mt-1 rounded-full overflow-hidden">
                                                    <div 
                                                        ...
                                                        style={{ width: `${Math.min(100, (Math.abs(t.cumulativeSum) / 10000000) * 100)}%` }}
                                                    />
                                                </div>
                                            </TableCell>
                                        </TableRow>
                    ...
                    <div className="p-4 bg-white border-t flex items-center justify-between shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
                        <div className="flex items-center gap-6">
                            ...
                            <div className="flex items-center gap-6">
                                <div className="flex flex-col gap-1">
                                    <span className="text-[9px] font-black text-rose-400 uppercase tracking-[0.2em] leading-none">TOTAL DEBT (GROSS)</span>
                                    <span className="text-[13px] font-black text-rose-600 tabular-nums leading-none">
                                        {formatMoneyVND(transactions.reduce((acc, t) => (t.type === 'debt' || t.type === 'expense') ? acc + t.amount : acc, 0))}
                                    </span>
                                </div>
                                <div className="flex flex-col gap-1 border-l pl-6">
                                    <span className="text-[9px] font-black text-emerald-400 uppercase tracking-[0.2em] leading-none">TOTAL REPAID</span>
                                    <span className="text-[13px] font-black text-emerald-600 tabular-nums leading-none">
                                        {formatMoneyVND(transactions.reduce((acc, t) => (t.type === 'repayment' || t.type === 'income') ? acc + t.amount : acc, 0))}
                                    </span>
                                </div>
                                <div className="flex flex-col gap-1 border-l pl-6">
                                    <span className="text-[9px] font-black text-amber-400 uppercase tracking-[0.2em] leading-none">CASHBACK TOTAL</span>
                                    <span className="text-[13px] font-black text-amber-600 tabular-nums leading-none">
                                        {formatMoneyVND(transactions.reduce((acc, t) => acc + t.cashback, 0))}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-1">
                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] leading-none">NET REMAINS</span>
                                <span className={cn(
