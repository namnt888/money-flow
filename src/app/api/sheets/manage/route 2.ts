...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!syncResult.success) {
      const upstreamError = syncResult.message ?? 'Sync failed'
      const normalizedError = upstreamError.includes('Cycle tag must be YYYY-MM')
        ? `Sheet upstream rejected cycle tag: raw=${rawCycle || '(empty)'} resolved=${normalizedCycle || '(empty)'} masterSheet=${isMasterSheet ? 'on' : 'off'}`
        : upstreamError
      return errorResponse(
        requestId,
        'sync_transactions',
